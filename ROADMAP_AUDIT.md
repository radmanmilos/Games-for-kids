...
- 2026-09-26 — **Task 112 (DEVICE-001) IMPLEMENTED.** Created `game/styles/viewport.css` — global `overflow-x:hidden`, `.ps-page` safe-area-aware wrapper with `env(safe-area-inset-*)`, `.ps-canvas-fill` for canvas games, `.ps-rotate-hint` for landscape-only games. Created `game/shared/viewport.js` — shared helpers: `safeArea()`, `resizeCanvas(canvas)`, `onResize(fn)` (debounced resize+orientation), `rotateHint(el)`. Wired into all 13 game pages. Zero behavior changes — shared layer only. `node --check` 68 files OK. NOT committed — user approves commit + sync.
