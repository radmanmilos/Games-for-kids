...
- Tasks 1–34, 38–52, **53 (Писање Tracing game)**, **54 (tools/ tooling)**, **43 (alphabet pronunciation/image audit)**, **55 (Phase 3: Учионица kids tier)**, and **57 (GitHub Pages via `docs/` mirror)** are DONE. **Task 56 (GitHub Actions auto-deploy) is ABANDONED** (2026-08-04, per user decision — Pages refused to deploy from `game/`; the workflow was removed). Tasks 35/36 (stickers + screen transitions) are **deferred to a later phase**. **Phase 3 is ACTIVE (2026-08-04) — the kids tier is built; the next item is the user's play-test + polish.**
