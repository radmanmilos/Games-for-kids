...
- Tasks 1–34, 38–52, **53 (Писање Tracing game)**, **54 (tools/ tooling)**, **43 (alphabet pronunciation/image audit)**, **55 (Phase 3: Учионица kids tier)**, **57 (GitHub Pages via `docs/` mirror)**, and **58 (Клавир Piano game)** are DONE. **Task 56 (GitHub Actions auto-deploy) is ABANDONED** (2026-08-04, per user decision — Pages refused to deploy from `game/`; the workflow was removed). Tasks 35/36 (stickers + screen transitions) are **deferred to a later phase**. **Phase 3 is ACTIVE (2026-08-04) — the kids tier is built; the next item is the user's play-test + polish. Phase 4 (new game set) has started with task 58 (Клавир/Piano) — next game to be picked by the user from the Future Mini Games list.**
...
- Games shipped: Animals, Shape Match, Match Game (candy), Paper Kitty Adventure (embedded in-app), Animal Scene Puzzle, Animal Counting, Animal Memory, Coloring, **Учионица (Classroom)**, **Писање (Tracing)**, **Клавир (Piano)**.
