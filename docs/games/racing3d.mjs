...
function main() {
  ...
  const r3dCfg = window.RACING3D_CONFIG || {
    obstacles: {},
    worlds: {},
    kartColors: [],
  };
  const WORLD_LIST =
    Array.isArray(sharedCfg.worlds) && sharedCfg.worlds.length
      ? sharedCfg.worlds
      : [
          {
            key: "meadow",
  ...
  function loadSave() {
    try {
      ...
      if (!s || typeof s.wins !== "number")
        return { wins: 0, world: 0, kart: 0 };
  ...
  function persistSave(s) {
    ...
    try {
      localStorage.setItem(
        SAVE_KEY,
        JSON.stringify({ wins: s.wins, world: w, kart: k }),
      );
    } catch (e) {
      /* private mode */
    }
  ...
  for (let i = 0; i < N; i++) {
    ...
    const hillY =
      (Math.sin(ang * 2) * 4.6 + Math.sin(ang * 4) * 2.1) * (world.hill || 1) +
      (rand() - 0.5) * 2.6;
  ...
  const groundYAt = (p, offset) =>
    p.y - 0.03 - Math.max(0, Math.abs(offset) - ROAD_HALF) * TERRAIN_SLOPE;
  {
  ...
  function ribbonDash(count, width, yOffset) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const r0 = new THREE.Vector3()
        .crossVectors(curve.getTangentAt(t0), UP)
        .normalize();
      const r1 = new THREE.Vector3()
        .crossVectors(curve.getTangentAt(t1), UP)
        .normalize();
      const l0 = p0.clone().addScaledVector(r0, -width / 2);
      l0.y += yOffset;
      const r0p = p0.clone().addScaledVector(r0, width / 2);
      r0p.y += yOffset;
      const l1 = p1.clone().addScaledVector(r1, -width / 2);
      l1.y += yOffset;
      const r1p = p1.clone().addScaledVector(r1, width / 2);
      r1p.y += yOffset;
      const b = i * 4;
      pos.push(
        l0.x,
  ...
  scene.add(
    new THREE.Mesh(ribbonVerts(0.4, -(ROAD_HALF - 0.55), 0.035), edgeMat),
  );
  // finish line
  {
  ...
  // flat red/white edge rumble strips (replaces tall curbs — classic racetrack edge feel)
  {
    ...
    const rumbleBand = (lat) =>
      ribbonVerts(0.6, lat, 0.04, (t) =>
        Math.floor(t * 70) % 2 === 0 ? red : white,
      );
  ...
  // --- scenery: trees ---
  {
    ...
    for (let i = 0; i < 46; i++) {
      ...
      trunk.position.y = gy + 1.5 * s;
      trunk.scale.setScalar(s);
      const crown = new THREE.Mesh(crownGeo, crownMat);
      crown.position.copy(base);
      crown.position.y = gy + (1.5 + 3) * s;
  ...
  // --- scenery: distance-readable landmarks (rocks, bushes, flower patches, flags, start-gate arch) ---
  {
    ...
    for (let i = 0; i < 26 && pi < PATCH_N; i++) {
      ...
      for (let k = 0; k < 5 && pi < PATCH_N; k++) {
        ...
        patches.setColorAt(
          pi,
          new THREE.Color(petalColors[(i + k) % petalColors.length]),
        );
        pi++;
      }
    }
    ...
    // start-gate arch (tall landmark over the finish line — readable from far)
    {
      ...
      lBase.y += 3.75;
      rBase.y += 3.75;
      const gL = new THREE.Mesh(gateGeo, gateMat);
      gL.position.copy(lBase);
      scene.add(gL);
      const gR = new THREE.Mesh(gateGeo, gateMat);
      gR.position.copy(rBase);
      scene.add(gR);
      const bannerMat = new THREE.MeshBasicMaterial({
        color: world.finishColor || 0xffd23f,
      });
      const banner = new THREE.Mesh(
        new THREE.BoxGeometry(ROAD_WIDTH + 3.6, 1.1, 0.25),
        bannerMat,
      );
      ...
      // checkered strip on banner
      {
        ...
        for (let ix = 0; ix < 16; ix++)
          for (let iy = 0; iy < 4; iy++) {
            g.fillStyle = (ix + iy) % 2 ? "#222" : "#eee";
            g.fillRect(ix * 8, iy * 4, 8, 4);
          }
        const tx = new THREE.Mesh(
          new THREE.BoxGeometry(ROAD_WIDTH + 3.0, 0.38, 0.26),
          new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(cv) }),
        );
        tx.position.copy(p0);
        tx.position.y = 7.85;
  ...
  let mapCanvas = null,
    ...
    mapPts = [],
    mapScale = 1,
    lastMapMarker = { t: 0, x: 0, y: 0 };
  if (mapEl) {
    const MAP_STEPS = 140;
    const loopPts = [];
    for (let j = 0; j <= MAP_STEPS; j++)
      loopPts.push(curve.getPointAt(j / MAP_STEPS));
    mapCanvas = document.createElement("canvas");
    mapEl.appendChild(mapCanvas);
    function layoutMap() {
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      const cw = Math.max(64, mapEl.clientWidth),
        ch = Math.max(64, mapEl.clientHeight);
      ...
      const midX = (minX + maxX) / 2,
        midZ = (minZ + maxZ) / 2;
      mapPts = loopPts.map((p) => ({
        x: cw / 2 + (p.x - midX) * mapScale,
        y: ch / 2 + (p.z - midZ) * mapScale,
      }));
    }
    layoutMap();
  ...
  function drawMap() {
    ...
    const c = mapCtx,
      w = mapCanvas.width / Math.min(window.devicePixelRatio || 1, 2);
    c.clearRect(0, 0, w, w);
    c.lineCap = "round";
    c.lineJoin = "round";
    ...
    c.lineWidth = roadW * 0.55;
    c.stroke();
    const t0 = mapPts[0],
      t1 = mapPts[1] || mapPts[0];
    const dx = t1.x - t0.x,
      dz = t1.y - t0.y;
    const L = Math.hypot(dx, dz) || 1;
    const nx = (-dz / L) * roadW * 0.48,
      ny = (dx / L) * roadW * 0.48;
    c.beginPath();
    c.moveTo(t0.x + nx, t0.y + ny);
    c.lineTo(t0.x - nx, t0.y - ny);
    c.strokeStyle = "#222";
    c.lineWidth = Math.max(2, roadW * 0.5);
    c.stroke();
    c.beginPath();
    c.moveTo(t0.x + nx, t0.y + ny);
    c.lineTo(t0.x - nx, t0.y - ny);
    c.strokeStyle = "#eee";
    c.lineWidth = Math.max(1, roadW * 0.5) / 2.4;
    c.stroke();
    const fi = show * (mapPts.length - 1);
    const i0 = Math.floor(fi),
      i1 = Math.min(mapPts.length - 1, i0 + 1),
      f = fi - i0;
    const mk = {
    ...
    c.beginPath();
    c.arc(mk.x, mk.y, Math.max(4, w * 0.035), 0, Math.PI * 2);
    c.fillStyle = "#e52521";
    c.fill();
  ...
  const kartMat = new THREE.MeshPhongMaterial({
    color: world.kartColor,
    shininess: 90,
    emissive: 0x120000,
  });
  const accentMat = new THREE.MeshPhongMaterial({
    color: world.kartAccent,
    shininess: 70,
    emissive: 0x141000,
  });
  ...
  const wheelMat = new THREE.MeshPhongMaterial({
    color: world.wheelColor,
    shininess: 30,
  });
  const hubMat = new THREE.MeshPhongMaterial({
    color: world.hubColor,
    shininess: 80,
    emissive: 0x666666,
  });
  ...
  const studMat = new THREE.MeshPhongMaterial({
    color: world.boostColor || 0xffd23f,
    emissive: 0x664400,
    shininess: 40,
  });
  const wheels = [];
  ...
  [
    ...
    ring.position.add(
      new THREE.Vector3(p[0] > 0 ? -1 : 1, 0, 0).multiplyScalar(0.14),
    );
    wg.add(ring);
    const h = new THREE.Mesh(hubGeo, hubMat);
    h.position.add(
      new THREE.Vector3(p[0] > 0 ? -1 : 1, 0, 0).multiplyScalar(0.19),
    );
    wg.add(h);
  ...
  const blobShadowMat = new THREE.SpriteMaterial({
    map: blobShadowTex,
    transparent: true,
    depthWrite: false,
    fog: false,
  ...
  function buildPickupMesh(kind, pColor, finColor) {
    ...
    if (kind === "shell") {
      const shell = new THREE.Mesh(
        ...
        mat(pColor, 0x221a00, 70),
      );
      shell.position.y = 0.5;
      shell.scale.set(1, 0.72, 0.8);
      g.add(shell);
      const fan = new THREE.Mesh(
        new THREE.ConeGeometry(0.3, 0.34, 10),
        mat(pColor, 0x221a00, 70),
      );
      fan.position.set(0, 0.62, -0.24);
      fan.rotation.x = Math.PI / 2 - 0.5;
      g.add(fan);
    } else if (kind === "snowflake") {
      for (let k = 0; k < 3; k++) {
        const arm = new THREE.Mesh(
          new THREE.BoxGeometry(0.72, 0.08, 0.14),
          mat(pColor, 0x00a0e0, 60),
        );
        arm.position.y = 0.5;
        arm.rotation.y = (k / 3) * Math.PI;
        g.add(arm);
      }
    } else if (kind === "candy") {
      const stick = new THREE.Mesh(
        new THREE.CylinderGeometry(0.07, 0.07, 0.62, 8),
        mat(0xffffff),
      );
      ...
      const head = new THREE.Mesh(
        ...
        mat(pColor, 0x552200, 90),
      );
      head.position.y = 0.72;
      g.add(head);
      const swirl = new THREE.Mesh(
        new THREE.TorusGeometry(0.34, 0.07, 8, 16),
        mat(0xffffff, 0, 30),
      );
      swirl.position.y = 0.72;
      swirl.scale.set(1, 1.35, 1);
      g.add(swirl);
    } else if (kind === "gem") {
      const gem = new THREE.Mesh(
        new THREE.OctahedronGeometry(0.42, 0),
        mat(pColor, 0x0a220a, 120),
      );
      gem.position.y = 0.62;
      g.add(gem);
    } else if (kind === "star") {
      const star = new THREE.Mesh(
        ...
        mat(pColor, 0x664400, 90),
      );
      star.position.y = 0.62;
      star.scale.set(1, 1.15, 0.45);
      g.add(star);
    } else if (kind === "moon") {
      const moon = new THREE.Mesh(
        new THREE.SphereGeometry(0.4, 14, 10),
        mat(pColor, 0x553300, 70),
      );
  ...
  const pickupBase = buildPickupMesh(
    world.pickup,
    world.pickupColor,
    world.finishColor,
  );
  ...
  const boostTex = (() => {
    ...
    const g = cv.getContext("2d");
    const bg =
      "#" + (world.boostColor || 0xffd23f).toString(16).padStart(6, "0");
    g.fillStyle = bg;
  ...
  for (let i = 0; i < BOOST_COUNT; i++) {
    ...
    const mesh = new THREE.Mesh(
      new THREE.BoxGeometry(2.2, 0.04, 2.6),
      boostMat,
    );
  ...
  // --- obstacles ---
  const obstacles = [];
  const OBS_COUNT = Math.max(
    6,
    Math.min(16, Math.round((base.obstacleDensity || 0.005) * 1700)),
  );
  for (let i = 0; i < OBS_COUNT; i++) {
    ...
    const mesh = (() => {
      ...
      for (let x = 0; x < 16; x++) {
        g.fillStyle =
          x % 2 === 0
            ? "#" + def.base.toString(16).padStart(6, "0")
            : "#ffffff";
    ...
    obstacles.push({
      ...
      cooldownUntil: 0,
      t,
      hitText: def.hitText || def.label + "! Брзина смањена.",
    });
    addBlobShadow(
      def.size * (type === "puddle" ? 1.1 : 1.4),
      base.clone().add(new THREE.Vector3(0, 0.05, 0)),
    );
  }
  ...
  function confettiBurst(pos) {
    for (let i = 0; i < 110; i++) {
      ...
      spawnP({
        pos,
        col: [0xff4040, 0x40a0ff, 0xffd23f, 0x40e060, 0xff88cc, 0xff9933][
          i % 6
        ],
        vel: new THREE.Vector3(
          Math.cos(ang) * sp,
  ...
  function flamePuff(exhaustPos) {
    // batch 6: longer/hotter boost flames — faster rearward vel (−3.2→−7),
    // longer max (0.3→0.42), bigger scale (1.1→2.0). Only ever called while
    // now < boostUntil, so this IS the boost visual identity.
    for (let e = 0; e < 2; e++) {
      const ep = exhaustPos[e] || exhaustPos[0];
      spawnP({
        pos: ep.clone(),
        col: [0xff6622, 0xffaa22][Math.random() > 0.5 ? 0 : 1],
        vel: new THREE.Vector3(
          (Math.random() - 0.5) * 2,
          0.9 + Math.random() * 1.6,
          -3.2 - Math.random() * 3.8,
  ...
  function makeNoiseBuffer(audio, dur) {
    const b = audio.createBuffer(
      1,
      Math.ceil(audio.sampleRate * dur),
      audio.sampleRate,
    );
  ...
  function startMusic() {
    ...
    musicStep = 0;
    try {
      musicStepTime = window.ctx().currentTime + 0.05;
    } catch (e) {
      return;
    }
      ...
      while (musicStepTime < horizon) {
        ...
        if (mel !== null && mel !== undefined) {
          ...
          g.gain.exponentialRampToValueAtTime(
            0.001,
            musicStepTime + eighth * 0.9,
          );
          o.start(musicStepTime);
          o.stop(musicStepTime + eighth * 0.95);
        }
        if (musicStep % 2 === 0) {
          ...
          bg.gain.exponentialRampToValueAtTime(
            0.001,
            musicStepTime + eighth * 1.8,
          );
          b.start(musicStepTime);
          b.stop(musicStepTime + eighth * 1.85);
        }
        if (m.ambient && Math.random() < m.ambient.rate)
          playAmbient(m.ambient.sound, m.ambient.vol);
        musicStep++;
  ...
  if (nextWorldBtn) {
    nextWorldBtn.addEventListener("click", () => {
      save = {
        wins: save.wins,
        world: (worldIdx + 1) % WORLD_LIST.length,
        kart: kartIdx,
      };
      persistSave(save);
      location.reload();
  ...
  function renderPicker() {
    if (worldGrid && worldGrid.children.length) {
      ...
      Array.from(kartRow.children).forEach((b, i) =>
        b.classList.toggle("sel", i === pendingKart),
      );
  ...
  if (startBtn) {
    startBtn.addEventListener("click", () => {
      const needsReload =
        pickerMidGame || pendingWorld !== worldIdx || pendingKart !== kartIdx;
      if (needsReload) {
  ...
  function update(dt, now) {
    ...
    if (decorAmp > 0 && decorBils.length) {
      ...
      for (const d of decorBils) {
        ...
        if (d.pulseAge >= 0) {
          d.pulseAge += dt;
          const ph =
            d.pulseAge <= 0.3 ? Math.sin((d.pulseAge / 0.3) * Math.PI) : 0;
          d.spr.scale.set(
            d.baseScale * (1 + 0.15 * ph),
            d.baseScale * (1 + 0.15 * ph),
            1,
          );
    ...
    if (steer !== 0) {
      steerYawValue +=
        (steer * STEER_YAW_MAX - steerYawValue) * (1 - Math.exp(-8 * dt));
      steerDrive +=
        (steer * STEER_YAW_MAX - steerDrive) * (1 - Math.exp(-8 * dt));
    } else {
      ...
      if (weatherAcc > 0.15) {
        ...
        for (let w = 0; w < 2 && particles.length < MAX_PARTICLES - 30; w++) {
          ...
          if (world.weather === "snow") {
            ...
            spawnP({
              pos: p2,
              col: [0xffd23f, 0xd8e0ff, 0xffffff][
    ...
    if (mode === "drive") {
      ...
      for (const b of boostPads) {
        ...
        if (
          now >= boostUntil &&
          kart.position.distanceTo(b.mesh.position) < b.radius
        ) {
      ...
      if (driftNow) {
        ...
        if (skidAcc > 0.05) {
          ...
          skidPuff(
            kart.position
              .clone()
              .addScaledVector(right, -steer * 1.0)
              .add(new THREE.Vector3(0, 0.3, -2)),
          );
        }
      }
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "ArrowLeft" || e.key === "a" || e.key === "A") {
      keys.left = true;
      recentPress.left = performance.now();
    }
    if (e.key === "ArrowRight" || e.key === "d" || e.key === "D") {
      keys.right = true;
      recentPress.right = performance.now();
    }
    startEngine();
  });
  ...
  window.__r3d = {
    ...
    audioState: () => {
      try {
        return audioCtxRef ? audioCtxRef.state : "noctx";
      } catch (_) {
        return "none";
      }
    },
    ...
    decorNear: (t) => {
      ...
      return best >= 0
        ? { idx: best, t: decorBils[best].t }
        : { idx: -1, t: -1 };
    },
    decorAt: (i) => {
      const d = decorBils[i];
      return d ? { t: d.t, baseScale: d.baseScale } : null;
    },
    decorAmp: () => decorAmp,
    setDecorAmp: (v) => {
      decorAmp = v ? 1 : 0;
