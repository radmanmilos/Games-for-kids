...
A full 3D (WebGL / three.js, r169 vendored in `game/assets/lib/`) racing game, built after the MK World / Crash Nitro Kart visual-feel analysis in `RACING_VISUAL_STYLE_ANALYSIS.md`. The 2D racer above is kept as-is and both are in the games hub.

- Vertical slice (task 100): new hub entry `racing3d` → `pages/racing3d.html` + `games/racing3d.mjs` + `games/racing3d-config.js` (meadow world only for now — more worlds port in later, config-driven).
- Chunkier cartoon kart (bright primaries, big rounded wheels, rim-light sheen, suspension bounce + turn banking), procedural hilly closed-loop track with red/white curbs and a checkered start line, gradient sky dome + depth fog.
- Chase cam with lag, look-ahead, speed FOV kick and slight roll; countdown 3-2-1-Крени! with Serbian speech; 🌸 flower pickups; 💧 puddle / 🚧 barricade obstacles that slow the kart (no fail state); 3-lap finish → win modal, wins persisted in `racing3dSave`.
- Offline-ready: three.js is bundled, cache list + offline manifest regenerated by `tools/build_offline.ps1`.
- Validation: `node tools/racing3d_smoke.js` (7/7 PASS) and `node tools/hub_smoke.js` (ALL PASS, 10 games buttons).
- Tuning so far (2026-09-11, from user play-test feedback): `MAX_SPEED` 132→70; **quaternion-slerp chase cam** (no more upside-down flip on hills, hills tamed); **`DoubleSide` fix** — the road/dashes/edges ribbons were wound down-facing and back-face culled (the road was literally invisible; root cause, not a color issue); road is now a classic two-lane asphalt with dashed center line + solid white edges; camera pulled in tight; kart holds its line when released + slip-angle body yaw.
