...
- **WebGL context loss (2026-09-25, batch 11):** three.js already skips rendering while the context is lost and re-initialises it on restore, but the child saw a frozen black screen and the kart teleported on recovery. The game now shows "Графика се привремено…" and freezes the simulation until the context returns.
- Validation: `node tools/racing3d_smoke.js` (17/17 PASS) and `node tools/hub_smoke.js` (ALL PASS, 10 games buttons; the 2D-racer button is `hidden` but still wired).
