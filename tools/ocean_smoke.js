/* Океан (Ocean) smoke test — Phase 4 adventure engine (fly mode).
   ...
   steering moves the fish, clamps hold, coins collect, obstacles knock the
   fish back, patrolling sharks move, goal completes the level, worlds picker +
...
(async () => {
  const h = await start({ page: '/pages/ocean.html', tag: 'ocean-smoke', width: 1100, height: 700 });
  ...
  await h.evalv(`(() => {
    ...
    a.obstacles.push({ x: a.player.x - 10, y: a.player.y - 10, width: 90, height: 90, type: 'rock' });
    a.update();
    return true;
  })()`);
  ...
  check('obstacle hit: fish knocked up (respawn), level not completed', hj.bumps >= 1 && hj.done === false, hit);

  const patrol = await h.evalv(`(() => {
    const a = window.__adv;
    a.loadWorld();
    a.obstacles.length = 0;
    const o = { x: 1500, y: 200, width: 40, height: 70, type: 'shark', vx: 2, minX: 1400, maxX: 1600 };
    a.obstacles.push(o);
    const x0 = o.x;
    a.update();
    const x1 = o.x;
  ...
  check('patrolling shark: moves forward, bounces at maxX and minX (vx flips)', pj.x1 > pj.x0 && pj.vx1 === 2 && pj.x2 >= pj.maxX && pj.vx2 < 0 && pj.x3 <= pj.minX && pj.vx3 > 0, patrol);
  ...
  check('goal reached: fish stops, win modal shown', goj.done === true && goj.pxAfter === goj.stoppedX && goj.shown === true && goj.title.indexOf('ПРЕЂЕН') !== -1 && goj.btn.indexOf('СЛЕДЕЋИ') !== -1, go);
