...
| `racing3d_smoke.js` | Canonical validation for the 3D Мала тркачица 3Д (three.js racer, task 100) — 7 checks: boots to `window.__r3d`, canvas + Serbian HUD (Ливада / Круг 1/3 / flowers), countdown overlay 3→Крени!, countdown → drive, steering moves the kart laterally + holds its line when released (reverses on the other key), scene really draws geometry (`tris() > 1000`), page chrome (back/restart/zones). |
