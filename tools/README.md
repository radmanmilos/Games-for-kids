...
| `racing3d_smoke.js` | Canonical validation for the 3D Мала тркачица 3Д (three.js racer, task 100) — **15 checks**: boots to `window.__r3d`, canvas + Serbian HUD (Ливада / Круг 1/3 / flowers), countdown overlay 3→Крени!, countdown → drive, **steering: wheels turn in + car drifts, car stops while wheels still return slowly, no side-drift, reverses** (sampled right after release while the wheels are still unwinding — the lateral position must not keep moving), boost pads spawned on road lanes (6), boost trigger raises the speed target + spawns flame particles, **wheels roll forward around their axle (rotation.x, no vertical wobble)**, drift engages while steering at speed (skid smoke) + disengages on release, **kart banks into the turn + front wheels point (steer yaw) while held, then both straighten to forward on release; wheels carry a 16-stud spin pattern (`__r3d.spokes()`)**, edge rumble + offroad dust (past the rumble strip the kart rumbles, clears on return), **race-track mini-map renders the loop with a live moving kart marker**, **hills clearly visible (≥6 height range) and the road always clears the terrain (never under it)**, scene really draws geometry (`tris()` > 1000), page chrome (back/restart/zones). |
