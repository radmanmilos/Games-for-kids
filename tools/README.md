...
| `adventure_smoke.js` | Canonical validation for the shared adventure engine (`game/games/adventure.js`) — 24 checks: drive mode (driving.html), ground mode (dino.html), and fly mode (ocean.html) each verify boot, mode, player spawn, draw safety, coin pickup, obstacle knockback, goal trigger, and resize re-anchor; static checks verify hub button, navigation route, and standalone boot wiring. |
